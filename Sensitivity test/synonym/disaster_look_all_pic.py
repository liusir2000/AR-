...
def getTQ_albef_feature_extractor_base(filename, model, vis_processors, txt_processors):
    disaster_classes = ["flood", "hail", "haze", "strong wind", "mudslide", "snow", "lightning", "tornado", "waterlogging"]
    disaster_descriptions = ['a photo of '+ adisaster +' disaster.' for adisaster in disaster_classes]
...
def getTQ_clip_general(filename, model, vis_processors, txt_processors, model_name):
    texts = ["flood", "hail", "haze", "strong wind", "mudslide", "snow", "lightning", "tornado", "waterlogging"]
    
    raw_image = Image.open(filename).convert("RGB")
    image = vis_processors["eval"](raw_image).unsqueeze(0).to(device)

    text_descriptions = [f"a photo of {t} disaster." for t in texts]
...
def getTQ_albef_vqa_vqav2(filename, model, vis_processors, txt_processors):
    questionStr = "what weather disaster is this ?"
    ...
    answer = model.predict_answers(
        samples={"image": image, "text_input": question}, answer_list=["flood", "hail", "haze", "strong wind", "mudslide", "snow", "lightning", "tornado", "waterlogging"],
